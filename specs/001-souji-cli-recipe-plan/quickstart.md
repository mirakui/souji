...
`souji init` provisions that directory and drops a commented-out template at
`default.rb` — the scenario `souji plan` reads when you give it no argument:

```bash
souji init
$EDITOR ~/.config/souji/scenario/default.rb
```

This walkthrough uses a named scenario instead, so that both forms are covered.
Create it at the conventional location:
...
| `scenario file not found: .../scenario/default.rb` (exit 2) | Bare `souji plan` looks for the `default` scenario | Run `souji init` and edit the template, or name a scenario: `souji plan weekly`. |
| `souji plan` right after `souji init` reports 0 items | The generated template is entirely commented out by design | Uncomment the `target` / `recipe` lines you want. |
